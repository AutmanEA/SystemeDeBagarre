...
@export var thickness: float = 850.0
...
func _ready() -> void:
	# Connexion des signaux de souris ET de clic
	input_event.connect(_on_input_event)
	mouse_entered.connect(_on_mouse_entered)
	mouse_exited.connect(_on_mouse_exited)
	
	# On applique l'épaisseur au démarrage
	sprite_top.position.y = -thickness
	sprite_border.position.y = -thickness
	sprite_thickness.position.y = -thickness
	
	
	# On aligne la collision à mi-hauteur
	$BaseCollision.position.y = -thickness
	$Figurine.position.y = -thickness
